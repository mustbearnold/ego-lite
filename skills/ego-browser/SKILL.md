...
ego-browser gives AI agents a CLI-accessible Node.js runtime with Playwright-style
`page`, `browser`, and `taskSpaces` facades plus `cdp`. Agents use them to
observe pages, interact with UI, evaluate browser-side JavaScript, and drive a
real browser for any web automation task.

For setup, install, or connection problems, read `references/install.md`.

Use the `Bash` tool to run all browser operations via an `ego-browser <<'EOF' ... EOF`
heredoc. Do not write code to a `.js` file first.
...
The heredoc body runs as a Node.js script that controls the selected ego-browser
task space. The facades and `cdp` are preloaded into that script; `console.log`
is the output channel.

## Common helpers

- Task spaces: `taskSpaces.list`, `taskSpaces.useOrCreate`, `taskSpaces.new`, `taskSpaces.claim`, `taskSpaces.switch`, `taskSpaces.handOff`, `taskSpaces.takeOver`, `taskSpaces.waitForAgentControl`, `taskSpaces.complete`
- Navigation / state: `browser.listTabs`, `browser.openOrReuseTab`, `browser.closeTab`, `browser.currentTab`, `browser.switchTab`, `browser.ensureRealTab`, `page.goto`, `page.info`, `page.url`, `page.title`
- Observation: `page.snapshot`, `page.snapshotRaw`, `page.screenshot`, `page.drainEvents`
- Locators and actions: `page.locator`, `page.getByRole`, `page.getByText`, `page.getByLabel`, `page.getByPlaceholder`, `page.mouse`, `page.keyboard`
- Wait: `page.waitForTimeout`, `page.waitForLoadState`, `page.waitForSelector`, `page.waitForFunction`, `page.waitForURL`, `page.waitForRequest`, `page.waitForResponse`, `page.waitForEvent`
- Fetch: `fetch.server`, `fetch.browser`
- CDP / evaluate: `page.evaluate`, `cdp`
- Output: `console.log`, `help`

Notes:
- `console.log(value)` — prints to the terminal; it is the output channel for a heredoc.
- `await page.info()` — normally resolves to `{ url, title, w, h, sx, sy, pw, ph }`; if a native browser dialog is open, it resolves to `{ dialog: ... }` instead because page JavaScript is blocked.
...
- `help(name)` — returns usage for a given facade, e.g. `console.log(help('page'))`.
...
```js
// DOM scroll and wait for a page condition
...
Locator methods such as `click`, `hover`, `dragTo`, `fill`, `press`, and
`screenshot` accept CSS, XPath, `@N` refs, and stable `loc=...` values from
`page.snapshot()` (`loc=role:...`, `loc=href:...`, and related locators).
`@N` refs are for ego-browser actions only; they are not valid selectors inside
`document.querySelector(...)`.

`page.mouse.click`, `page.mouse.dblclick`, `page.mouse.move`, and
`page.mouse.drag` use CSS-pixel coordinates. Locator actions are preferred for
ordinary controls; use coordinates when the target is visual or canvas-based.

- `page.mouse.click(420, 260)` — click a viewport point.
- `page.mouse.click('@21')` — click a snapshot ref or locator target.
- `page.locator('button.primary').click()` — click a strict locator.
- `page.mouse.drag([[420, 260], [620, 260]])` — drag through viewport points.

```js
await page.locator('@21').click()
await page.locator('button.primary').click()
await page.mouse.click(420, 260)
await page.locator('@5').hover()
await page.mouse.drag([[fromX, fromY], [toX, toY]])
```
...
`page.evaluate()` runs an expression or function in the current page. Function
closures are not captured; pass data explicitly through the supported argument
channel when using a function. Use `cdp()` for raw browser-protocol operations.

When you need to run multi-step logic inside the browser, wrap it in a single
self-invoking closure and return once:
...
1. **Semantic workflow: `page.snapshot()` + refs / locators** — default for most pages with normal text, links, buttons, forms, tables, and lists.
   ...
   - Act with `await page.locator('@N').click()`, `await page.locator('@N').fill(...)`, or stable `loc=...` values. Use direct DOM logic only when it is simpler than helper calls.
...
2. **Visual workflow: `await page.screenshot()` + coordinate/keyboard actions** — use when the page is primarily visual, canvas-like, heavily virtualized, or when accessibility / semantic structure is incomplete.
   - Inspect the screenshot, act with viewport coordinates such as `await page.mouse.click(x, y)`, `await page.mouse.dblclick(x, y)`, `await page.keyboard.press(...)`, and `await page.keyboard.type(...)`, then verify with another screenshot or a reliable export/readback path.
...
- All current Playwright-style `timeout`, `settle`, polling, and `page.waitForTimeout` values are in **milliseconds**.
...
- `@N` refs are only valid for the most recent `page.snapshot()` call — every call rebuilds the ref map. Ref numbers come from the CDP `backendNodeId`, so the same element may keep the same number across calls; but to use `@N`, N must appear in the latest snapshot output. An element scrolled out of the viewport, a DOM re-render, or a previous call with `scope: 'only_within_viewport'` that did not cover the element can all cause `Unknown ref`. For elements you need to reference long-term, use the `loc=...` value from snapshot output as a stable selector, or write a CSS selector directly.
- On Linux, snapshots also walk nested and cross-origin iframe frames. Actionable refs from a child frame retain their frame ownership and can be passed directly to helper actions; use a fresh ref after navigation or DOM changes. Stable `loc=role:...` and link `loc=href:...` locators are resolved across the page's frame tree, while a fresh child-frame ref remains the strictest choice when acting on a known element.
- `page.evaluate()` returns the evaluated result, not a JSON string — do not wrap it with `JSON.parse(...)`.
- Inside a `page.evaluate()` template string, regex backslashes must be doubled (e.g. `\\d`, `\\s`), or use `String.raw`.
- If `page.info()` reports `w: 0` or `h: 0`, do not continue coordinate actions or screenshots until the viewport is fixed. Try switching to the real tab, reloading, or using CDP viewport metrics, then verify with `await page.info()` and `await page.screenshot()`.
