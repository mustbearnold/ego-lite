...
- On Linux, snapshots also walk nested and cross-origin iframe frames. Actionable refs from a child frame retain their frame ownership and can be passed directly to helper actions; use a fresh ref after navigation or DOM changes. Stable `loc=role:...` locators currently describe the top document, so prefer the child-frame ref when acting inside an iframe.
