...
- On Linux, snapshots also walk nested and cross-origin iframe frames. Actionable refs from a child frame retain their frame ownership and can be passed directly to helper actions; use a fresh ref after navigation or DOM changes. Stable `loc=role:...` locators are resolved across the page's frame tree, while a fresh child-frame ref remains the strictest choice when acting on a known element.
