...
| AppleScript application/window/tab/bookmark automation | Implemented as a portable AppleScript-style subset over the typed contract | Linux now exposes `--applescript` for one common command per invocation, including `tell application` blocks, application/window/tab/bookmark specifiers, `get`, `count`, `exists`, `set`, `open`, `print`, `save`, `execute`, and the covered standard-suite mutations. The adapter runs through the same authenticated bridge or standalone CDP dispatcher and is covered by detached source/Electron tests. Native AppleScript parsing, implicit coercion, multi-statement execution, and native macOS print-dialog semantics remain platform-specific. |
