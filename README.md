...
The upstream ego lite app runs on macOS. This checkout also adds a Linux host that
uses an installed Chromium/Chrome binary with the same `ego-browser` SDK and task-space
workflow. Windows remains on the [roadmap](https://lite.ego.app/roadmap).
...
Linux needs Node.js 22+, Chromium or Google Chrome, and a graphical desktop session for a visible browser. From this checkout:

```bash
sh skills/ego-browser/scripts/install.sh
```

The installer builds the SDK, installs a user-local host under `~/.local/share/ego-lite`, adds `ego-browser` and `ego-lite` to `~/.local/bin`, and registers a desktop launcher. On an interactive first install it offers to migrate one detected Chromium-family profile before launch; use `EGO_LITE_SKIP_MIGRATION=1` for unattended setup. Use `--profile NAME` or `EGO_LITE_PROFILE_ID` to select another persistent Linux profile, and `--server-name NAME` to isolate a named browser instance. Use `EGO_LITE_HEADLESS=1` for CI or a headless machine. The desktop launcher accepts HTTP(S) URLs and local files through `ego-lite --launch`; see [`platform/linux/README.md`](platform/linux/README.md) for Linux-specific behavior and environment variables.

For a self-contained desktop package with embedded Chromium, run `npm install` and `npm run package:linux` from [`platform/electron/`](platform/electron/). This builds an AppImage, Debian package, and RPM; the packaged executable accepts `--cli --profile NAME --server-name NAME`, includes the Linux host, SDK, and skill resources, offers a one-time welcome page and first-launch migration with explicit selection when multiple Chromium-family profiles are found, exposes a profile switcher, imported and editable bookmarks, extension controls, download status, and opt-in browser-data sync (automatic sync waits for its interval and is skipped when ego lite is the system default browser), supports non-persistent private tabs, persists primary tabs and native window state across restarts, exposes toolbar Import data for a later migration, and supports background AppImage updates for tagged releases.
...
On first macOS launch, ego lite asks whether to migrate your Chrome data. The Linux host uses its own persistent Chromium profile under `~/.local/share/ego-lite/chromium-profile`; select a separate profile with `ego-lite --profile work --doctor` or migrate into it with `ego-lite --profile work --migrate-profile --from "$HOME/.config/google-chrome"` before first use if you want to bring across portable browser data, cookies, restorable HTTP(S) tabs/tab groups, and passwords from Chromium’s basic plaintext store, then log in there once for anything not migrated. In the Electron package, use the toolbar’s Import button to repeat that migration later; the app restarts safely around the import and preserves a backup of replaced target data. Browser-internal pages and keyring-backed passwords remain separate.
...
The Linux port uses Chromium's standard accessibility tree for snapshots and backend-node refs, including nested/cross-origin iframe content, viewport/full-page scope, action marks, and stable role/href locators plus link URLs across the page's frame tree. The Electron shell exposes a persistent tab strip with grouped-tab accents, direct Space reveal controls, normal tab creation/closing, and Ctrl/Cmd+Shift+T reopening of recently closed normal tabs, including their tab-group metadata; Agent and private tabs remain isolated from that user-tab history. The closed-source macOS app's custom snapshot engine is not part of this repository, so snapshot wording and some browser chrome details can differ while the helper API remains the same.
...
| **Experience accumulation that makes your agent faster the more you use it** _(coming soon)_ | Most of an agent's time on browser tasks goes to trial and error. ego lite's official Skill distills every successful action into reusable tools and workflows, so similar tasks down the line run up to 5x faster.                                                                                                                                                                                                                                                           |
