...
tab editing/navigation, and the covered standard-suite mutations. Collection
properties support `every`, ordinal selectors support `first` and `last`, and
scoped reads/counts retain their parent object. Commands in a block execute
sequentially; the final response includes `script.statements` when more than
one command ran. Queries return `result.value`; mutating commands retain the
typed automation result. Native AppleScript parsing, full implicit
