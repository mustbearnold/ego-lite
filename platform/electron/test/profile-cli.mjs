...
const electronArguments = packagedExecutable
  ? ["--cli", "--profile", "work", "--doctor"]
  : ["platform/electron", "--cli", "--profile", "work", "--doctor"];
const profileRoot = await mkdtemp(join(tmpdir(), "ego-electron-profile-cli-"));
...
function parseReport(stdout) {
  ...
  for (const start of starts.reverse()) {
    try {
      ...
      // Electron may have written diagnostic output after the report.
    }
  }
...
function runCli() {
  ...
  delete environment.EGO_LITE_PROFILE_DIR;
  delete environment.EGO_LITE_STATE_PATH;
...
try {
  ...
  assert.equal(
    result.code,
    0,
    `packaged CLI failed (${result.signal || "no signal"})\n${result.stdout}\n${result.stderr}`,
  );
  ...
  assert.equal(
    report.statePath,
    join(stateHome, "ego-lite", "profiles", "work", "task-spaces.json"),
  );
  console.log(
    JSON.stringify({
      profileDir: report.profileDir,
      statePath: report.statePath,
      tabCount: report.tabs?.length || 0,
