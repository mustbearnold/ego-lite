...
const profileDir = await mkdtemp(join(tmpdir(), "ego-electron-fullscreen-"));
...
async function sendNativeKey(key) {
  ...
  await execFileAsync("xdotool", ["key", "--window", windowId, key]);
}
...
try {
  ...
  await waitFor("primary tab", async () => {
  ...
  try {
    ...
    const state = () =>
      connection.request(
        "Runtime.evaluate",
        {
          expression: "window.egoLite.getBrowserState().then((state) => state.fullscreen)",
          awaitPromise: true,
          returnByValue: true,
        },
        attached.sessionId,
    ...
    await sendNativeKey("F11");
    const entered = await waitFor("fullscreen enter", async () =>
      (await state()) === true,
    );
    await sendNativeKey("F11");
    const exited = await waitFor("fullscreen exit", async () =>
      (await state()) === false,
    );
    console.log(
      JSON.stringify({
